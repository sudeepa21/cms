...
// Fetch the logged-in lecturer's user_id from the session
$user_id = $_SESSION['user_id'];

// Fetch lecturer details from the 'users' table
$stmt = $conn->prepare("SELECT id, user_id, uni_ID, first_name, last_name, email, mobile, username, profile_picture FROM users WHERE user_id = ?");
if (!$stmt) {
...
if ($result->num_rows === 1) {
    $lecturer = $result->fetch_assoc(); // Fetch the lecturer's details
} else {
    die("Error: Lecturer profile not found for user_id = $user_id");
}
...
<head>
    ...
    <title>Lecturer Profile | Smart Campus</title>
        ...
        <form id="profileForm" class="bg-white p-4 rounded shadow-sm" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <div class="col-12 text-center">
                    <?php if (!empty($lecturer['profile_picture'])): ?>
                        <img src="../uploads/<?php echo htmlspecialchars($lecturer['profile_picture']); ?>" alt="Profile Picture" class="profile-picture">
                    <?php else: ?>
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <input type="text" id="uni_ID" class="form-control" value="<?php echo htmlspecialchars($lecturer['uni_ID']); ?>" readonly>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <input type="text" id="username" class="form-control" value="<?php echo htmlspecialchars($lecturer['username']); ?>" readonly>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <input type="text" id="mobile" name="mobile" class="form-control" value="<?php echo htmlspecialchars($lecturer['mobile']); ?>" required>
                    <small class="text-muted">Must be a 10-digit number starting with 0 (e.g., 0773456789).</small>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        $("#removeProfilePicture").click(function() {
            if (confirm("Are you sure you want to remove your profile picture?")) {
                $.post("update_lec_profile.php", { remove_profile_picture: true }, function(response) {
                    if (response.status === "success") {
                        $("#statusMessage").text(response.message).css("color", "green");
